...
#Creating a tables in Postgres SQL
async def db_migrate(config:Config)->None:
    pool=get_pool()
    async with pool.acquire() as conn:
        #It will create the vector extension in the database if it doesn,t exist
...
#Creating long term memory using Postgres SQL
...
#Searching long term memory using postgres SQL

async def ltm_search(config: Config, topic: str)->dict:
    pool=get_pool()
    embedding=await asyncio.to_thread(lambda:_model.encode(topic).tolist())
    async with pool.acquire() as conn:
...
async def ltm_search_releated(config: Config, topic: str)-> str |None:
    embedding=await asyncio.to_thread(lambda:_model.encode(topic).tolist())
    pool=get_pool()
    async with pool.acquire() as conn:
        row = await conn.fetchrow(
