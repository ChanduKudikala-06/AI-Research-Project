...
async def _rate_limit(request:Request):
    #It checks if user has exceeded max limit it will raise exception.
    #If count is 1 it will set expiration to 1 minute and it be cleared from redis
    ...
    if count>config.rate_limit_window:
...
async def _process_job(data:dict,msg_id:str):
    ...
    try:
        ...
        #Fetch the session history first
...
#dependencies it will check the whether valid api key is using and checks the rate limiting(how many requests are hitting)
